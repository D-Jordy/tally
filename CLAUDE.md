...
- **Bilingual UI**: every user-facing string goes through `__()` with a key in *both* `lang/en/` and `lang/nl/` — never hardcode either language in a Blade/PHP file. Locale comes from `Accept-Language` (`SetLocale` middleware), not a user setting; charts/`Number::` formatting follow `app()->getLocale()`. (Overrides my personal "UI strings are Dutch" default — this app is English-first with a Dutch translation.)
